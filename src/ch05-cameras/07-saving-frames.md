...
The shortest complete camera program there is: mount, wait for one frame, write it,
unmount. There is no loop, which makes it the right place to see what a `Frame` holds and
what it takes to get pixels out of the process.

## One frame, blocking

`mount_camera` has already waited for the stream to exist by the time it returns, but the
first frame still has to be rendered. Block for it rather than polling. From
`examples/rust/src/bin/ex14_camera_save.rs`:

```rust
// mount_camera already waited for the stream to exist, but the first frame
...
```cpp
// ===== the one frame =====
// mount_camera already waited for the stream to exist, but the first
...
Each surface asserts the same invariant in its own idiom, and C++ is the only one that
unwinds carefully: it unmounts the camera before returning, because there is no `?` to carry
the cleanup and no exception being thrown.
...
```text
frame seq=3 t=1.842s 1280x720 rgb8 (3 B/px, step=3840, 2764800 bytes)
...
`step` is `width * 3` exactly, and `data.len()` is `height * step` exactly. Nothing on this
path is padded or compressed.
...
- Channels are R, G, B in that order, which is what PPM wants. A format that wanted BGR
  would convert here, at the call site, and nowhere else.

`mono8` is expanded to grey by repeating the single byte three times, and `rgba8` drops
alpha by taking `pixel[..3]`.

The run ends by putting the robot back the way it was:
...
> **Gotcha.** The unmount is the last statement, so Ctrl-C during the two-second wait
> leaves `snapshot` mounted on a robot that outlives your process. Run the example again to
> reconfigure the same name, or restart the simulator.

## When one file is not enough

Writing images from a loop makes this the slow part of your program: a 720p rgb8 frame is
2.76 MB, and 60 of those per second is more than most disks want. For capture rather than
