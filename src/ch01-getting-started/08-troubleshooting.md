...
| | A vector was read in the wrong frame | Read `s.coord_frame_id`: the truck publishes `fru` and the multirotor `frd`, so the third component means opposite things | [Frames, axes and units](../ch02-concepts/07-frames-and-units.md) |
